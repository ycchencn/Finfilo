...
const limitUpDownData = ref({
    ...
    datasets: [
        {
            label: '涨停家数',
            data: [68, 72, 81, 56, 94, 102, 88],
            backgroundColor: '#ef4444',
        },
        {
            label: '跌停家数',
            data: [12, 8, 15, 9, 6, 10, 11],
...
<template>
    <div class="dashboard-container">
        ...
        <div class="grid-row">
            ...
            <Card class="chart-card">
                <template #title> 涨跌停家数</template>
                <template #content>
                    <div class="chart-wrapper">
                        <Chart type="bar" :data="limitUpDownData" :options="limitUpDownOptions"/>
                    </div>
                </template>
            </Card>
        </div>
        ...
        <Card class="stocks-card">
            <template #title> 领涨个股</template>
            <template #content>
                <DataTable :value="topStocks" stripedRows size="small" class="stocks-table">
                    <Column field="name" header="名称"/>
                    <Column field="code" header="代码"/>
                    <Column field="price" header="最新价"/>
                    <Column field="changePercent" header="涨跌幅">
                        <template #body="slotProps">
                            <Tag
                                :severity="slotProps.data.changePercent >= 0 ? 'danger' : 'success'"
                                :value="formatSign(slotProps.data.changePercent) + '%'"
                            />
                        </template>
                    </Column>
                </DataTable>
