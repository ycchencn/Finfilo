    ...
    <Fluid class="grid grid-cols-12 gap-8">
        ...
        <div class="col-span-12 xl:col-span-7">
            <div class="card flex flex-col">
                <div class="font-semibold text-xl mb-4">市场温度与板块机会（{{ market_temp_data_last_date }}）</div>
                <DataTable
                    dataKey="id"
                    :value="market_weights_list"
                    :paginator="false"
                    :rows="50"
                    :rowHover="true"
                    showGridlines
                    sortField="daily_change"
                    sortOrder="-1"
                >
                    <template #empty> No data found.</template>
                    <template #loading> Loading data. Please wait.</template>
                    <Column field="stock_code" filterField="stock_code" header="行业名称/指标" style="min-width: 8rem">
                        <template #body="{ data }">
                            {{ data.industry_name }}
                        </template>
                    </Column>
                    <Column field="change_pct" header="当日 %" bodyClass="text-center" style="min-width: 8rem">
                        <template #body="{ data }">
                            <ProgressBar200p :showValue="false" :barHeight="25" :value="data.daily_change.toFixed(2)" :times="10" readonly />
                        </template>
                    </Column>
                    <Column field="turnover_rate" header="本周 %" bodyClass="text-center" style="min-width: 8rem">
                        <template #body="{ data }">
                            <ProgressBar200p :showValue="false" :barHeight="25" :value="data.week_to_date.toFixed(2)" :times="10" readonly />
                        </template>
                    </Column>
                    <Column field="turnover_rate" header="本月 %" bodyClass="text-center" style="min-width: 8rem">
                        <template #body="{ data }">
                            <ProgressBar200p :showValue="false" :barHeight="25" :value="data.month_to_date.toFixed(2)" :times="10" readonly />
                        </template>
                    </Column>
                    <Column field="turnover_rate" header="今年 %" bodyClass="text-center" style="min-width: 8rem">
                        <template #body="{ data }">
                            <ProgressBar200p :showValue="false" :barHeight="25" :value="data.year_to_date.toFixed(2)" :times="1" readonly />
                        </template>
                    </Column>
                    <Column field="institution_participation" header="机会值" bodyClass="text-center">
                        <template #body="{ data }">
                            <Rating :modelValue="(data.opportunity_score / 20).toFixed(2)" readonly />
                        </template>
                    </Column>
                </DataTable>
            </div>
        ...
        <div class="col-span-12 xl:col-span-5">
            <div class="card flex flex-col items-center">
                <div class="font-semibold text-xl mb-4">市场温度分析报告（{{ market_temp_data_last_date }}）</div>
                <MarkdownRenderer :markdown="market_temp_data_last.ai_suggestion"></MarkdownRenderer>
            </div>
        </div>
    </Fluid>
</template>
